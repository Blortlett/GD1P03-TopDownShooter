...
void cDebugMenuUI::Update()
{
	// If options menu not open: return early! dont run function at all
	if (!cGameSettings::GetInstance().GetIsDebugMenuOpen()) return;
	...
	mInfiniteAmmo.Update(mWindow);
	mInfiniteAmmo.Draw(mWindow);

	mGodMode.Update(mWindow);
	mGodMode.Draw(mWindow);

	mSkipLevel.Update(mWindow);
	mSkipLevel.Draw(mWindow);
